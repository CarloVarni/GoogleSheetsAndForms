...
class Validator:
    ...
    def execute(self, CTX):
        if len(self.InputCollections) == 0:
            raise Exception(FAIL("No input collection has been specified for {0}".format(self.NAME)))
        if len(self.InputCollections) != len(self.ReferenceData):
            raise Exception(FAIL("Reference data size MUST match input collection size!"))
        
        print(NOTE("Validating Google Reader ..."))

        for i in range(0, len(self.InputCollections)):
            print("   \\__ Validating '{0}' collection".format(self.InputCollections[i]))
            inputs = CTX.retrieve(self.InputCollections[i])
            referenceData = self.ReferenceData[i]()
            try:
                self.compareCollections(inputs, referenceData)
                print("      \\__ Validation Status: {0}".format(OK("SUCCESS")))
            except Exception:
                print("      \\__ Validation Status: {0}\n".format(FAIL("FAILURE")))
                raise Exception(FAIL("GoogleReader Validation has Failed!"))
    ...
    def compareCollections(self, inputs, references):
        ...
        if len(inputs) != len(references):
            raise Exception(FAIL("Entries have different size w.r.t. reference entries!"))

        for i in range(0, len(inputs)):
            inputEntry = inputs[i]
            referenceEntry = references[i]

            if not isinstance(inputEntry, list):
                raise Exception(FAIL("Input is not a list!"))
            if len(inputEntry) != len(referenceEntry):
                raise Exception(FAIL("Entry has different size w.r.t. reference entry!"))
            
            for j in range(0, len(inputEntry)):
                if not isinstance(inputEntry[j], str):
                    raise Exception(FAIL("Input Entry is not a string!"))
                if inputEntry[j].strip() != referenceEntry[j].strip():
                    raise Exception(FAIL("Mismach in entries!"))
...
def retrieveFewDateInfo():
    return [
        ["2021-03-28T14:02:34Z", "2021-03-28T14:02:34Z"],
        ["2021-03-27T13:30:49Z", "2021-03-27T13:30:49Z"],
        ["2021-03-26T15:46:39Z", "2021-03-26T15:46:39Z"],
        ["2021-03-25T16:01:21Z", "2021-03-25T16:01:21Z"],
        ["2021-03-24T19:02:59Z", "2021-03-24T19:02:59Z"],
        ["2021-03-23T19:02:03Z", "2021-03-23T19:02:03Z"],
        ["2021-03-22T19:02:02Z", "2021-03-22T19:02:02Z"],
        ["2021-03-21T19:04:10Z", "2021-03-21T19:04:10Z"],
        ["2021-03-20T15:58:34Z", "2021-03-20T15:58:34Z"],
        ["2021-03-19T19:01:04Z", "2021-03-19T19:01:04Z"],
        ["2021-03-18T16:32:59Z", "2021-03-18T16:32:59Z"],
        ["2021-03-17T19:00:54Z", "2021-03-17T19:00:54Z"],
    ]
